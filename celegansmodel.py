# -*- coding: utf-8 -*-
"""
Created on Mon Jan 11 16:15:47 2021
...
class Protein_Drug_Prediction(object):
    def dense_Conv(self, size, filters, activation, initializer, regularizer_param):
        def f(inputs):
            model_yuan = inputs
    ...
    def model(self, dropout, filters, prot_len=2500, activation='relu', initializer="glorot_normal", drug_len1=2048,drug_len2=300):
        ...
        model_ps = [self.dense_Conv(stride_size, filters, activation, initializer, 0.0001)(model_p) for stride_size in (10,15)]
        if len(model_ps)!=1:
        ...
        model_ds = [self.dense_Conv(stride_size, filters, activation, initializer, 0.0001)(model_d) for stride_size in (3,3)]
        if len(model_ds)!=1:
...
protein = np.load("celegans/input/proteins.npy")
drug1 = np.load("celegans/input/morgan.npy")
drug2 = np.load("celegans/input/mol2vec.npy")
label = np.load("celegans/input/interactions.npy")
