...
class Protein_Drug_Prediction(object):
    ...
    def validation(self, drug_feature1,drug_feature2, protein_feature, label, test_drug1,test_drug2,test_pro,test_label,output_file=False, n_epoch=20, batch_size=32, **kwargs):
        AUC_best = 0
        for _ in range(n_epoch):
            history = self.__model_t.fit([drug_feature1,drug_feature2,protein_feature],label, epochs=_+1, batch_size=batch_size, validation_data=([test_drug1,test_drug2,test_pro],test_label),shuffle=True, verbose=1,initial_epoch=_)
...
train_protein = np.load("shuju/DB_train/proteins.npy")
train_drug1 = np.load("shuju/DB_train/morgan.npy")
train_drug2 = np.load("shuju/DB_train/mol2vec.npy")
train_label = np.load("shuju/DB_train/interactions.npy")
